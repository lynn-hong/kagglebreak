...
class Facebook_crawler():
    ...
    def get_fb_profile_pic(self):
        # ID, FACEBOOK URL
        fetch_sql = "SELECT m.id, c.info FROM MEMBER m, CONTACT c WHERE m.id=c.m_id AND c.contact_type=0 AND m.type=1;"
        update_sql = "UPDATE MEMBER SET picture='{}' WHERE id={};"
        id_regex = re.compile(r'www.facebook.com/(\d+)')
        ...
        for usr_id in id_list:
            if usr_id[1] != "":
                ...
                real_id = id_regex.search(usr_id[1]).group(1)
                url = "https://graph.facebook.com/{}/picture?access_token={}&redirect=false&width=500".\
                    format(real_id, self.access_token)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        mysql_file = "/home/lynn/workspace/django/kagglebreak/mysql.cnf"
